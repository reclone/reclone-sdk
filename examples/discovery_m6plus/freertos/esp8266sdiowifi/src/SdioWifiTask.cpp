   ...
   else
      {
         ...
         if (sdio_err != SD_OK && sdio_err != SD_REQUEST_PENDING)
         ...
         else
            {
               ...
               uint32_t retries;
               trace_printf (
                        "SD_CMD_SDIO_SEN_OP_COND succeeded! Response: 0x%08x, NumIO: %u, Mem: %u\n",
               ...
               if (sdio_err != SD_OK || retries >= 100)
               ...
               else
                  {
                     ...
                     trace_printf (
                              "SDIO card ready! CMD5 Response: 0x%08x, Retries: %u\n",
                              resp4, retries);
...
HAL_SD_ErrorTypedef
SdioWifiTask::Cmd0Error ()
{
   HAL_SD_ErrorTypedef sdio_err = SD_OK;
   uint32_t timer = 0;

   while ((timer < SDIO_CMD0TIMEOUT)
            && (__SDIO_GET_FLAG(SDIO, SDIO_FLAG_CMDSENT) == RESET))
      {
         __NOP ();
         ++timer;
      }
...
HAL_SD_ErrorTypedef
SdioWifiTask::Cmd3Resp6Error (uint16_t & rca)
{
   HAL_SD_ErrorTypedef sdio_err = SD_OK;
   uint32_t timer = 0;
   uint16_t new_rca = 0;
